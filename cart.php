...
<header>
    <menu>
    <p><a href="category.php">Товары</a></p>
        <p class="logo"><a href="index.php" style="font-family: var(--font-family);
                font-weight: 900;
                font-size: 36px;
                text-align: center;
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                clearCartAndReload(); // Вызов функции для очистки корзины и перезагрузки страницы
...
function clearCartAndReload() {
    ...
    xhr.open("GET", "clearCart.php", true); // Замените clearCart.php на URL, обрабатывающий очистку корзины
    xhr.send();
}
